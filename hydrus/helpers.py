...
def set_response_headers(resp: Response,
    ...
    resp.headers['Link'] = '<{}{}/vocab>; rel="http://www.w3.org/ns/hydra/core#apiDocumentation"'.format(get_hydrus_server_url(),get_api_name())
    return resp
...
def hydrafy(object_: Dict[str, Any], path: Optional[str]) -> Dict[str, Any]:
    ...
    if path == object_["@type"]:
        object_["@context"] = '/{}/contexts/{}.jsonld'.format(get_api_name(),object_["@type"])
    else:
        object_["@context"] = "/{}/contexts/{}.jsonld".format(get_api_name(),path)
    return object_
