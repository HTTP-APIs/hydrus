...
from hydrus.samples.hydra_doc_sample import doc as api
import random
...
class TestCRUD(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        id_list = api["@id"].split('/')
        id_list.pop()
        id_list.pop()
        host = '/'.join(id_list)[0:]
        self.HYDRUS_SERVER_URL = host+'/'
        self.API_NAME = api["@id"].split('/')[len(api["@id"].split('/')) - 2] + '/'
    ...
    def test_insert(self):
        """Test CRUD insert."""
        object_ = gen_dummy_object(self.test_class, self.doc)
    ...
    def test_update(self):
        """Test CRUD update."""
        object_ = gen_dummy_object(self.test_class, self.doc)
        new_object = gen_dummy_object(self.test_class, self.doc)
        id_ = "30"
    ...
    def test_insert_ids(self):
        ...
        for index in range(len(ids.split(','))):
            object = gen_dummy_object(self.test_class, self.doc)
